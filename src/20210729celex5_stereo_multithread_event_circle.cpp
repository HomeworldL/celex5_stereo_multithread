...
public:
  ...
  std::vector<double> vec_disp;
  std::vector<double> vec_disp_fit;
  ...
  int  SpatioTemporalMatch(Circle2D &circle, geometry_msgs::PoseStamped &pose);
  void StereoMatch(cv::Point2f &uvLeft,cv::Point2f &uvRight, geometry_msgs::PoseStamped &pose);
  void StereoDispDeep(cv::Point2f &uvLeft,cv::Point2f &uvRight, geometry_msgs::PoseStamped &poses);
...
public:
  Celex5StereoMTEventsCircle(ros::NodeHandle& nh0, ros::NodeHandle& nh1)
  {
    ...
    disp_outfile.open("disp.txt", ios::out);  

    if(!master_outfile.is_open () || !slave_outfile.is_open () || !disp_outfile.is_open () )
    {
    ...
    const celex5_msgs::EventVector &msg, region_of_interest &roi,int index) 
{
  ...
  std::cout<<"block_vote[20]"<<block_vote[20]<<std::endl;
            ...
            else//这个时候才拟合
            {
              ...
              if(slave_circle.r > 10  && slave_circle.r < 28) // 之前是80
              {
                ...
                //   slave_circle.y = LPF_alpha*slave_circle.y + (1-LPF_alpha)*slave_circle_queue[slave_circle_queue.size()-1].y;
                  ...
                  int ret = SpatioTemporalMatch(slave_circle,pose_msg);
...
//
int Celex5StereoMTEventsCircle::SpatioTemporalMatch(Circle2D &circle, geometry_msgs::PoseStamped &pose_msg)
{
  if(master_circle_queue.empty())
  {
    return 0;
  ...
  while(1)
  {
    ...
    else if(circle.time < master_circle_queue[find_index].time)
    {
      ...
      else
      {
        ...
        slave_point.x = circle.x;
        slave_point.y = 800 - circle.y - 1;
        master_point.x = master_x_insert;
        master_point.y = 800 - master_y_insert - 1;
        pose_msg.header.stamp.sec = floor(circle.time/1e6);
        pose_msg.header.stamp.nsec = (circle.time%1000000)*1e3;

        std::cout<<"matched row: "<<circle.y<<"----"<<master_circle_queue[near_index_front].y<<"//"<<master_circle_queue[near_index_back].y<<std::endl;
        // point_pair.time = circle.time;

        // disp_outfile << master_point.x - slave_point.x << std::endl;
        // vec_disp.push_back(master_point.x - slave_point.x);
    ...
    else // time == time
    {
      ...
      slave_point.x = circle.x;
      slave_point.y = 800 - circle.y - 1;
      master_point.x = master_circle_queue[near_index_front].x; 
      master_point.y = 800 - master_circle_queue[near_index_front].y - 1;// 我不知道这个地方要不要800 - y - 1;
      pose_msg.header.stamp.sec = floor(circle.time/1e6);
      pose_msg.header.stamp.nsec = (circle.time%1000000)*1e3;
...
//************************************
void Celex5StereoMTEventsCircle::StereoMatch(cv::Point2f &uvLeft,cv::Point2f &uvRight, geometry_msgs::PoseStamped &poses)
{
    //  [u1]      |X|                      [u2]      |X|
    //Z*|v1| = Ml*|Y|                    Z*|v2| = Mr*|Y|
    //  [ 1]      |Z|                      [ 1]      |Z|
    ...
    //世界坐标系中坐标
    poses.pose.position.x = XYZ.at<float>(0,0)/1000;
    // poses.pose.position.y = 0.1 * XYZ.at<float>(1,0)/1000 + 0.9 * world_y_last;
    poses.pose.position.y = XYZ.at<float>(1,0)/1000;
    poses.pose.position.z = XYZ.at<float>(2,0)/1000;

    world_y_last = poses.pose.position.y;
}///
void Celex5StereoMTEventsCircle::StereoDispDeep(cv::Point2f &uvLeft,cv::Point2f &uvRight, geometry_msgs::PoseStamped &poses)
{
  int u0_error = 5;// u0-u1

  // if (disp_num == 0)
  // {
  //   disp = uvLeft.x-uvRight.x;
  // }
  // else
  // {
    disp = uvLeft.x-uvRight.x ;
  ...
  disp_outfile << disp << "," << X-disp << std::endl;

  vec_disp.push_back(disp) ;
  vec_disp_fit.push_back(X) ;
  ...
  // std::cout<<"mLeftIntrinsic.at<float>(0,0) = "<<mLeftIntrinsic<<std::endl;
  ...
  if (disp_num == 0)
  {
    z_camera_left = fx*b/(disp); 
    x_camera_left = (uvLeft.x-u0)*b/(disp);
    y_camera_left = (uvLeft.y-v0)*b/(disp);
  }
  else
  {
    z_camera_left = 0.2*fx*b/(disp) + 0.8*z_camera_left_tmp; 
    x_camera_left = 0.2*(uvLeft.x-u0)*b/(disp) + 0.8*x_camera_left_tmp;
    y_camera_left = 0.2*(uvLeft.y-v0)*b/(disp) + 0.8*y_camera_left_tmp;
  }
  disp_num++;
  z_camera_left_tmp = z_camera_left;
  x_camera_left_tmp = x_camera_left;
  y_camera_left_tmp = y_camera_left;
  
  std::cout<<"x,y,z = "<<x_camera_left<<","<<y_camera_left<<","<<z_camera_left<<std::endl;
  ...
  std::cout<<"x,y,z = "<<XYZ_world(0,0)<<","<<XYZ_world(1,0)<<","<<XYZ_world(2,0)<<std::endl;

  //世界坐标系中坐标
  poses.pose.position.x = XYZ_world(0,0)/1000;
  poses.pose.position.y = XYZ_world(1,0)/1000;
  poses.pose.position.z = XYZ_world(2,0)/1000;
}
