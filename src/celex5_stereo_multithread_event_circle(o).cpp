...
public:
  ...
  std::vector<double> vec_z_camera_left;
  std::vector<double> vec_z_camera_left_fit;
...
public:
  Celex5StereoMTEventsCircle(ros::NodeHandle& nh0, ros::NodeHandle& nh1)
  {
    ...
    z_camera_left_outfile.open("z_camera_left.txt", ios::out);  
    ...
    const celex5_msgs::EventVector &msg, region_of_interest &roi,int index) 
{
  ...
  // std::cout<<"block_vote[10]"<<block_vote[10]<<std::endl;
    ...
    if(msg.events[i].y < master_row_last )//如果小于上一次的行数，则说明发生下一次行循环,开始对上一次循环处理，理论上可以不用等循环完，但我写不来程序
    {
        ...
        //当roi内的事件大于10才拟合，每拟合一次更新一次ROI
        {
            ...
            else//这个时候才拟合
            {
              ...
              master_roi.r = 1.5*(master_circle.r+5);
    ...
    if(msg.events[i].y < slave_row_last )//如果小于上一次的行数，则说明发生下一次行循环,开始对上一次循环处理，理论上可以不用等循环完，但我写不来程序
    {
        ...
        //当roi内的事件大于10才拟合，每拟合一次更新一次ROI
        {
            ...
            else//这个时候才拟合
            {
              ...
              slave_roi.r = 1.5*(slave_circle.r+5);
...
void Celex5StereoMTEventsCircle::StereoDispDeep(cv::Point2f &uvLeft,cv::Point2f &uvRight, geometry_msgs::PoseStamped &poses)
{
  ...
  vec_z_camera_left.push_back(z_k);
  vec_z_camera_left_fit.push_back(X);

  z_camera_left_outfile << X << "," << z_k << std::endl;

  x_camera_left = (uvLeft.x-u0)*z_camera_left/fx;
  y_camera_left = (uvLeft.y-v0)*z_camera_left/fx;
...
void Celex5StereoMTEventsCircle::PlotTrajectory2D()
{
  ...
  plt::figure_size(1280, 800);
  // Plot line from given x and y data. Color is selected automatically.
  plt::plot(vec_z_camera_left,".");
  plt::plot(vec_z_camera_left_fit,"r");
  plt::xlim(0,1280);
  plt::title("z_camera_left");
  filename = "./vec_z_camera_left.png";
  std::cout << "Saving result to " << filename << std::endl;
  plt::save(filename);
